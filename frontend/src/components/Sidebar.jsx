  ...
  return (
    <div style={styles.sidebar} className="glass">
      ...
      {(() => {
        ...
        const isExecutive = userRoleLower.includes('delivery head') || userRoleLower.includes('ceo');

        return (
          <div style={styles.footer}>
            <div 
              style={{ ...styles.userInfo, cursor: isExecutive ? 'default' : 'pointer' }}
              onClick={() => {
                if (!isExecutive && onOpenScorecard) {
                  onOpenScorecard(currentUser);
                }
              }}
              title={isExecutive ? `${currentUser?.name} (${currentUser?.role})` : "Click to view My Performance Scorecard"}
              className={isExecutive ? "" : "user-profile-btn"}
            >
